...
def check_values(value, other_inputs):
    missing_value = 2020 - value
    if missing_value in other_inputs:
        logging.info(f'{missing_value}, {value}')
...
def run():
    ...
    for idx, value in enumerate(inputs):
        ...
        other_values = inputs[0:idx] + inputs[idx+1:input_count]
        result = check_values(value, other_values)
        if result:
            return result
