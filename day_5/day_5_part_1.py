...
def find_seat(boarding_pass) -> Tuple[int, int]:
    column = (7, 0)
    row = (127, 0)
    ...
    for instruction in boarding_pass:
        if instruction == 'F':
            total = (row[0]+ row[1])
            row = (int(total/ 2), row[1])
        elif instruction == 'B':
            total = (row[0] + row[1])
            row = (row[0], int(total / 2))
        elif instruction == 'L':
            total = (column[0] + column[1])
            column = (int(total / 2), column[1])
        elif instruction == 'R':
            total = (column[0] + column[1])
            column = (column[0], int(total / 2))
...
def run() -> int:
    ...
    max_seat_id_seen: int = 0
    for boarding_pass in boarding_passes:
        row, column = find_seat(boarding_pass)
        seat_id = calculate_seat_id(row, column)
        max_seat_id_seen = max(max_seat_id_seen, seat_id)

    return max_seat_id_seen


if __name__ == '__main__':
    max_seat_id = run()
    print(f'highest_seat_id: {max_seat_id}')
