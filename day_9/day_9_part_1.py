...
def _is_sum_of_seen_values(value, preamble):
    for idx, seen in enumerate(preamble):
        match = value - seen
        if match in preamble[0:idx-1] + preamble[idx:]:
            return True
    return False
...
def find_invalid_number(lines):
    ...
    while len(remaining):
        ...
        if not _is_sum_of_seen_values(value, preamble):
            return value
...
def run() -> int:
    ...
    return find_invalid_number(lines)
